...
let allGermanyFederalStates = [],
  allGermanCountyDetails = [],
  allGermanyCounties = [],
  allGermanyCities = [];
// These flags gate Germany city/county loading, which is currently disabled (APIs removed in 0.9).
// They are still written below but only read from commented-out code, hence the disable.
/* eslint-disable @typescript-eslint/no-unused-vars */
let allGermanyFederalStatesLoaded = null,
  allGermanyCountiesLoaded = null,
  allGermanyCitiesLoaded = null;
...
class Covid19 extends utils.Adapter {
   ...
   */
  constructor(options) {
    // @ts-expect-error super options typing
    super({
      ...options,
  ...
  async onReady() {
    try {
      ...
      const selectedGermanyFederalStates =
        this.config.selectedGermanyFederalStates || [];
      ...
      this.log.debug(
        `Configuration object before config load : ${JSON.stringify(this.config)}`,
      );

      // Determine if routine must run to get data for tables
      const loadedArrays = await this.getObjectAsync(
        `${this.namespace}.countryTranslator`,
      );
      if (!loadedArrays) {
        allGermanyCitiesLoaded = false;
      } else {
        allGermanyCitiesLoaded = loadedArrays.native.allGermanyCities || false;
        allGermanyCountiesLoaded =
          loadedArrays.native.allGermanyCounties || false;
        allGermanyFederalStatesLoaded =
          loadedArrays.native.allGermanyFederalStates || false;
      }
      ...
      const loadCountries = async () => {
        try {
          ...
          try {
            ...
            this.log.debug(
              `Data from COVID-19 API received : ${JSON.stringify(apiResult)}`,
            );
            this.log.debug(
              `load all country's : ${this.config.loadAllCountrys} as ${typeof this.config.loadAllCountrys}`,
            );
          } catch (error) {
            this.log.warn(
              `[loadCountries] Unable to contact COVID-19 API : ${error}`,
            );
            return;
          }
          ...
          for (const countryData of apiResult) {
            ...
            const countryObject = this.getCountryObject(
              countryData.country,
            ...
            this.log.debug(
              `api name: ${countryData.country}, converted name: ${countryName}, dp name: ${country}, continent: ${continentName}`,
            );
            ...
            if (countryIsChecked) {
              try {
                ...
                await this.writeVaccinationDataForCountry(
                  country,
                  await VaccinationService.getVaccinationDataByIsoCode(
                    vaccinationData$,
                    countryObject.code.iso3,
                  ),
                );
                if (country === "Germany") {
                  await this.writeHospitalDataForId(
                    country,
                    await HospitalService.getGermanOverallHospitalData(
                      germanHospitalData$,
                    ),
                  );
                  await this.writeHospitalIndexDataForId(
                    country,
                    await HospitalIndexService.getGermanHospitalIndexByFederalState(
                      germanHospitalIndexData$,
                      "Bundesgebiet",
                    ),
                  );
                }
              ...
              if (this.config.deleteUnused === true) {
                await this.localDeleteState(country).catch(() => void 0); // ignore error
              }
            }
            ...
            continentsStats[continentName] =
              continentsStats[continentName] || {};
            continentsStats[continentName]["countries"] =
              continentsStats[continentName]["countries"] || []; // collect all countries of continent
            continentsStats[continentName]["inhabitants"] =
              continentsStats[continentName]["inhabitants"] || 0; // inhabitants to calculate relative values
            continentsStats["America"]["countries"] =
              continentsStats["America"]["countries"] || [];
            continentsStats["America"]["inhabitants"] =
              continentsStats["America"]["inhabitants"] || 0;
            continentsStats["World_Sum"]["inhabitants"] =
              continentsStats["World_Sum"]["inhabitants"] || 0;

            continentsStats[continentName]["countries"].push(countryName);
            continentsStats[continentName]["inhabitants"] =
              continentsStats[continentName]["inhabitants"] +
              countryData["cases"] / countryData["casesPerOneMillion"];
            continentsStats["World_Sum"]["inhabitants"] =
              continentsStats["World_Sum"]["inhabitants"] +
              countryData["cases"] / countryData["casesPerOneMillion"];
            ...
            for (const property of Object.keys(countryData)) {
              ...
              continentsStats[continentName][property] =
                continentsStats[continentName][property] || 0;
              continentsStats["America"][property] =
                continentsStats["America"][property] || 0;
              continentsStats["World_Sum"][property] =
          ...
          // Write Top 5
          this.log.debug(
            `Top 5 Countries : ${JSON.stringify(apiResult.slice(0, 5))}`,
          );
          for (let position = 1; position <= 5; position++) {
            ...
            this.log.debug(
              `Country loop rank : ${position} ${JSON.stringify(country)}`,
            );
          ...
          for (const continentsStatsKey in continentsStats) {
            this.log.debug(
              `${continentsStatsKey}: ${JSON.stringify(continentsStats[continentsStatsKey])}`,
            );

            await this.setObjectNotExistsAsync(
              `global_continents.${continentsStatsKey}`,
              {
                type: "channel",
                common: {
            ...
            for (const val in continentsStats[continentsStatsKey]) {
              if (val === "countryInfo") {
                await this.localDeleteState(
                  `global_continents.${continentsStatsKey}.${val}`,
                );
              }
              if (
                Object.prototype.hasOwnProperty.call(
                  continentsStats[continentsStatsKey],
                  val,
                ) &&
                val !== "countryInfo" &&
                val !== "inhabitants" &&
                ...
                if (
                  val !== "countries" &&
                  val !== "casesPerOneMillion" &&
                  val !== "deathsPerOneMillion"
                ) {
                  await this.localCreateState(
                    `global_continents.${continentsStatsKey}.${val}`,
                    val,
                    continentsStats[continentsStatsKey][val],
                  );
                  ...
                  await this.localCreateState(
                    ...
                    val,
                    (
                      continentsStats[continentsStatsKey]["cases"] /
                      continentsStats[continentsStatsKey]["inhabitants"]
                    ).toFixed(2),
                  );
                  ...
                  await this.localCreateState(
                    ...
                    val,
                    (
                      continentsStats[continentsStatsKey]["deaths"] /
                      continentsStats[continentsStatsKey]["inhabitants"]
                    ).toFixed(2),
                  );
                } else {
                  await this.localCreateState(
                    `global_continents.${continentsStatsKey}.${val}`,
                    val,
                    continentsStats[continentsStatsKey][val].join(),
                  );
                }
              } else if (
                Object.prototype.hasOwnProperty.call(
      ...
      // eslint-disable-next-line @typescript-eslint/no-unused-vars -- dormant: Germany federal-state API removed in 0.9, kept for future re-enable
        ...
        try {
          ...
          try {
            apiResult = await axios
              ...
              .then((response) => response.data);
            this.log.debug(
              `Data from RKI Corona Bundesländer API received : ${JSON.stringify(apiResult)}`,
            );
            this.log.debug(
              `load all country's : ${this.config.loadAllCountrys} as ${typeof this.config.loadAllCountrys}`,
            );
            ...
            this.log.warn(
              `[germanyBundesland] Unable to contact Corona Bundesländer API : ${error}`,
            );
            return;
          }
          ...
          if (
            ...
            this.log.warn(
              `Incorrect data received from API Corona Bundesländer, values not updated`,
            );
            return;
          }
          ...
          try {
            vaccDataGermany = await axios
              .get("https://api.corona-zahlen.org/vaccinations")
              .then((response) => response.data);
            this.log.debug(
              `Data from api.corona-zahlen.org received : ${vaccDataGermany}`,
            );
          } catch (error) {
            this.log.warn(
              `[germanyBundesland] Unable to contact api.corona-zahlen.org : ${error}`,
            );
          }
          ...
          if (
            !vaccDataGermany ||
            typeof vaccDataGermany !== "object" ||
            !vaccDataGermany.data ||
            !vaccDataGermany.data.states
          ) {
            this.log.warn(
              `Incorrect data received from api.corona-zahlen.org received, values not updated`,
            );
            return;
          }
          ...
          for (const vaccStates in vaccDataGermany.data.states) {
            germanyVaccinationData[
              vaccDataGermany.data.states[vaccStates].name
            ] = {
              allVacc:
                vaccDataGermany.data.states[vaccStates]
                  .administeredVaccinations,
              firstVacc: vaccDataGermany.data.states[vaccStates].vaccinated,
              secondVacc:
                vaccDataGermany.data.states[vaccStates].secondVaccination
                  .vaccinated,
              firstVaccQuote:
                vaccDataGermany.data.states[vaccStates].quote * 100,
              secondVaccQuote:
                vaccDataGermany.data.states[vaccStates].secondVaccination
                  .quote * 100,
            };
          ...
          for (const feature of apiResult.features) {
            this.log.debug(
              `Getting data for Federal State : ${JSON.stringify(feature.attributes.LAN_ew_GEN)}`,
            );
            ...
            await this.localDeleteState(
              `${channelName}._Impfungen.rkiImpfungenProTausend`,
            );
            await this.localDeleteState(
              `${channelName}._Impfungen.rkiDifferenzVortag`,
            );
            await this.localDeleteState(
              `${channelName}._Impfungen.rkiIndikationAlter`,
            );
            await this.localDeleteState(
              `${channelName}._Impfungen.rkiIndikationBeruf`,
            );
            await this.localDeleteState(
              `${channelName}._Impfungen.rkiIndikationMedizinisch`,
            );
            await this.localDeleteState(
            ...
            if (
              this.config.getAllGermanyFederalStates ||
              selectedGermanyFederalStates.includes(federalStateName)
            ) {
              ...
              try {
                await this.writeHospitalDataForId(
                  channelName,
                  await HospitalService.getGermanHospitalDataByFederalState(
                    germanHospitalData$,
                    federalStateName,
                  ),
                );
                ...
                this.log.debug(
                  `Cannot write hospital data for ${channelName}: ${error}`,
                );
              }
              ...
              try {
                ...
                this.log.debug(
                  `Cannot write hospital index data for ${channelName}: ${error}`,
                );
              }
              ...
              if (vaccDataGermany != null) {
                if (germanyVaccinationData[federalStateName]) {
                  ...
                  await this.localCreateState(
                    `${channelName}._Impfungen.rkiImpfungenGesamtVerabreicht`,
                    "Gesamtzahl bisher verabreichter Impfungen",
                    germanyVaccinationData[federalStateName].allVacc,
                  );
                  await this.localCreateState(
                    `${channelName}._Impfungen.rkiErstimpfungenKumulativ`,
                    "Erstimpfungen Kumulativ",
                    germanyVaccinationData[federalStateName].firstVacc,
                  );
                  await this.localCreateState(
                    `${channelName}._Impfungen.rkiZweitimpfungenKumulativ`,
                    "Zweitimpfungen Kumulativ",
                    germanyVaccinationData[federalStateName].secondVacc,
                  );
                  await this.localCreateState(
                    `${channelName}._Impfungen.rkiErstimpfungenImpfquote`,
                    "Erstimpfungen Impfquote",
                    await this.modify(
                      `round(2)`,
                      germanyVaccinationData[federalStateName].firstVaccQuote,
                    ),
                  );
                  await this.localCreateState(
                    `${channelName}._Impfungen.rkiZweitimpfungenImpfquote`,
                    "Zweitimpfungen Impfquote",
                    await this.modify(
                      `round(2)`,
                      germanyVaccinationData[federalStateName].secondVaccQuote,
                    ),
                  );

                  // Delete unused states from previous RKI version
                  await this.localDeleteState(
                    `${channelName}._Impfungen.rkiErstimpfungenBioNTech`,
                  );
                  await this.localDeleteState(
                    `${channelName}._Impfungen.rkiErstimpfungenModerna`,
                  );
                  await this.localDeleteState(
                    `${channelName}._Impfungen.rkiErstimpfungenAstraZeneca`,
                  );
                  await this.localDeleteState(
                    `${channelName}._Impfungen.rkiErstimpfungenDifferenzVortag`,
                  );
                  await this.localDeleteState(
                    `${channelName}._Impfungen.rkiZweitimpfungenKumulativ`,
                  );
                  await this.localDeleteState(
                    `${channelName}._Impfungen.rkiZweitimpfungenBioNTech`,
                  );
                  await this.localDeleteState(
                    `${channelName}._Impfungen.rkiZweitimpfungenModerna`,
                  );
                  await this.localDeleteState(
                    `${channelName}._Impfungen.rkiZweitimpfungenAstraZeneca`,
                  );
                  await this.localDeleteState(
                    `${channelName}._Impfungen.rkiZweitimpfungenDifferenzVortag`,
                  );
                }
              }
                ...
                switch (attributeName) {
                  case "Aktualisierung": //  Last refresh date
                    await this.localCreateState(
                      `${channelName}.updated`,
                      "updated",
                      feature.attributes[attributeName],
                    );
                    break;
                  ...
                  default:
                    this.log.debug(
                      `Data \\"${attributeName}\\" from API ignored having values : ${feature.attributes[attributeName]}`,
                    );
                }
              }
            } else {
          ...
          this.log.debug(
            `allGermanyFederalStates : ${JSON.stringify(allGermanyFederalStates)}`,
          );
      ...
      // eslint-disable-next-line @typescript-eslint/no-unused-vars -- dormant: Germany counties API removed in 0.9, kept for future re-enable
        ...
        try {
          ...
          try {
            apiResult = await axios.get(
              "https://services7.arcgis.com/mOBPykOjAyBO2ZKk/arcgis/rest/services/RKI_Landkreisdaten/FeatureServer/0/query?where=1%3D1&outFields=OBJECTID,GEN,BEZ,death_rate,cases,deaths,cases_per_100k,cases7_per_100k,cases_per_population,BL,county,last_update&returnGeometry=false&outSR=4326&f=json",
            );
            this.log.debug(
              `Data from RKI Corona Landkreise API received : ${apiResult.data}`,
            );
            this.log.debug(
              `load all country's : ${this.config.loadAllCountrys} as ${typeof this.config.loadAllCountrys}`,
            );
            ...
            this.log.warn(
              `[germanyBundesland] Unable to contact RKI Corona Bundesländer API : ${error}`,
            );
            return;
          }
          ...
          if (typeof values !== "object") {
            this.log.warn(
              `Incorrect data received from API RKI Corona Bundesländer, values not updated`,
            );
            return;
          }
          ...
          for (const feature of values.features) {
            ...
            this.log.debug(
              `Getting data for Landkreise : ${JSON.stringify(feature.attributes.county)} containing ${JSON.stringify(feature.attributes)}`,
            );
            ...
            if (countiesType === "Kreisfreie Stadt") {
              ...
              this.log.error(
                `Unknown ${countiesType} received containing ${JSON.stringify(feature)}`,
              );
            }
            ...
            const folderStructure = async () => {
              ...
              await this.extendObjectAsync(
                `Germany.${countiesType}.${countyName}`,
                {
                  type: "channel",
                  common: {
                    name: countyName,
            ...
            if (
              countiesType == "Kreis" &&
              (this.config.getAllGermanyCounties ||
                selectedGermanyCounties.includes(countyName))
            ) {
            ...
            if (
              countiesType == "Stadt" &&
              (this.config.getAllGermanyCities ||
                selectedGermanyCities.includes(countyName))
            ) {
              await folderStructure();
            ...
            for (const attributeName of Object.keys(feature.attributes)) {
              this.log.debug(
                `Statename will be : Germany.${countiesType}.${countyName} containing ${JSON.stringify(feature.attributes)}`,
              );
                ...
                switch (countiesType) {
                  case "Stadt":
                    if (
                      ...
                      await this.localCreateState(
                        `Germany.${countiesType}.${countyName}.${attributeName}`,
                        attributeName,
                        feature.attributes[attributeName],
                      );
                    }
                  ...
                  default:
                    await this.localDeleteState(
                      `Germany.county.${countyName}.${attributeName}`,
                    );
                }
              }
            }
          ...
          this.log.debug(
            `allGermanCountyDetails : ${JSON.stringify(allGermanCountyDetails)}`,
          );

          allGermanyCities = allGermanyCities.sort();
          this.log.debug(
            `allGermanyCities : ${JSON.stringify(allGermanyCities)}`,
          );

          allGermanyCounties = allGermanyCounties.sort();
          this.log.debug(
            `allGermanyCounties : ${JSON.stringify(allGermanyCounties)}`,
          );
      ...
      vaccinationData$ = VaccinationService.refreshVaccinationData().catch(
        (error) => this.log.warn(`Vaccination Data Warning: ${error}`),
      ...
      germanHospitalIndexData$ =
        HospitalIndexService.refreshGermanHospitalIndexData().catch((error) =>
          this.log.warn(`Hospital Index Data Warning: ${error}`),
        ); // load german hospital index
      ...
      this.terminate("Adapter closed unexpectedly, not all data processed");
    }
  }
  ...
  async localCreateState(state, name, value) {
    ...
    try {
      ...
      const writable =
        stateAttr[name] !== undefined ? stateAttr[name].write || false : false;
      const state_name =
      ...
      const unit =
        stateAttr[name] !== undefined ? stateAttr[name].unit || "" : "";
  ...
  getCountryObject(country, countryInfo) {
    try {
      ...
      countryObj = countryJs.findByName(
        country.replace(/_/g, " ").replace(/é/g, "e").replace(/ç/g, "c"),
      );
      ...
      if (country !== "Diamond Princess" && country !== "MS Zaandam") {
        this.log.warn(
          `${country} (iso2: ${countryInfo.iso2}, iso3: ${countryInfo.iso3}) not found in lib! Must be added to the country name translator.`,
        );
      }
    } catch (error) {
  ...
  async addUserCountriesTranslator() {
    try {
      const userCountryTranslator =
        await this.getStateAsync("countryTranslator");
        ...
        try {
          ...
          Object.keys(userCountries).forEach((countryId) => {
            if (
              ...
              this.log.info(
                `user defined country translation added: ${countryId} -> ${userCountries[countryId]}`,
              );
            }
          });
        } catch (parseError) {
          this.log.error(
            `Can not parse json string for user defined country translation! Check input of datapoint '.countryTranslator'. error: ${parseError.message}`,
          );
        }
      }
    } catch (error) {
  ...
  async writeVaccinationDataForCountry(country, data) {
    if (data) {
      ...
      this.log.debug(
        `Cannot write vaccination data for ${country}, if this error continues please report a bug to the developer! Totals: ${JSON.stringify(data)}`,
      );
    }
  }
  ...
  async writeHospitalDataForId(id, data) {
    if (!id || !data) {
      this.log.debug(
        `Cannot write hospital data for ${id}, if this error continues please report a bug to the developer! Totals: ${JSON.stringify(data)}`,
      );
      return;
    }
  ...
  async writeHospitalIndexDataForId(id, data) {
    if (!id || !data) {
      this.log.debug(
        `Cannot write hospital index data for ${id}, if this error continues please report a bug to the developer! Totals: ${JSON.stringify(data)}`,
      );
      return;
    }
  ...
  modify(method, value) {
    ...
    try {
      if (method.match(/^custom:/gi) != null) {
        ...
        value =
          Math.round(value * Math.pow(10, inBracket)) / Math.pow(10, inBracket);
