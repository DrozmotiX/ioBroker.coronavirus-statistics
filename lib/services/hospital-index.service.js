...
/** Service to fetch German hospital index (RKI) data. */
class HospitalIndexService {
  ...
  static refreshGermanHospitalIndexData() {
    return axios
      ...
      .then((data) =>
        data.filter(
          (item) => item.Datum === new Date().toLocaleDateString("fr-CA"),
        ),
      )
  ...
  static getGermanHospitalIndexByFederalState(data$, federalState) {
    return data$
      .then(
        (data) => data.filter((item) => item.Bundesland === federalState)[0],
      )
      .then((data) => {
      ...
      .catch((error) => {
        throw new Error(
          `Cannot get hospital index data for ${federalState} from rki: ${error}`,
        );
      });
  }
  ...
  static getGermanOverallHospitalIndex(data$) {
    return HospitalIndexService.getGermanHospitalIndexByFederalState(
      data$,
      "Bundesgebiet",
    );
  }
