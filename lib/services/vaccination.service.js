...
/** Service to fetch vaccination data (Our World in Data). */
  ...
  static refreshVaccinationData() {
    return (
      axios
        ...
        .catch((error) => {
          throw new Error(
            `Cannot get vaccination data from our world in data ${error}`,
          );
        })
    );
  }
    ...
    return (
      data$
        ...
        .then(
          (data) =>
            data.filter((item) =>
              item.iso_code.toLocaleLowerCase().includes(isoCode.toLowerCase()),
            )[0],
        )
        .then((data) => {
        ...
        .catch((error) => {
          throw new Error(
            `Cannot get vaccination data for ${isoCode} from our world in data ${error}`,
          );
        })
    );
  }
