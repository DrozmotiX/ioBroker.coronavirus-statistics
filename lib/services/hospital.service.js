...
const { aeRegex, oeRegex, ueRegex } = require("../regex");

/** Service to fetch German hospital (DIVI) data. */
class HospitalService {
    ...
    return axios
      ...
      .then((jsonData) => {
        jsonData.federalStates = jsonData.data.map((federalState) => ({
          ...federalState,
          bundesland: this.formatGermanHospitalDataFederalState(
  ...
  static getGermanHospitalDataByFederalState(data$, federalState) {
    return data$
      .then(
        (data) =>
          data.federalStates.filter(
            (federalStateObject) =>
              federalStateObject.bundesland === federalState,
          )[0],
      )
      .then((data) => {
      ...
      .catch((error) => {
        throw new Error(
          `Cannot get hospital data for ${federalState} from divi: ${error}`,
        );
      });
  }
}
